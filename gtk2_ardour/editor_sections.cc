...
EditorSections::redisplay ()
{
	...
	do {
		...
		if (l) {
			...
			location_row_map.insert (pair<ARDOUR::Location*, Gtk::TreeModel::iterator> (l, newrow));
		}
	} while (l);
	...
	do {
		...
		if (l) {
			if (start == selection.time.start_time() && end == selection.time.end_time()) {
				LocationRowMap::iterator map_it = location_row_map.find (l);
